	...
	run: async function (client, message, args) {
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			try {
				...
				let thing = new MessageEmbed()
					.setAuthor("Playlist has been added to queue", "https://cdn.discordapp.com/attachments/837747398926532609/1315432519310774362/Music.gif")
		...
		async function handleVideo(video, message, channel, playlist = false) {
			...
			if (!serverQueue) {
				...
				if (playlist) return;
				let thing = new MessageEmbed()
					.setAuthor("Song has been added to queue", "https://cdn.discordapp.com/attachments/837747398926532609/1315432519310774362/Music.gif")
...
async	function play(guild, song) {
  ...
  let afk = JSON.parse(fs.readFileSync("./afk.json", "utf8"));
    ...
    if (!song){
      if (!online.afk) {
        sendError("Leaving the voice channel because I think there are no songs in the queue. If you like the bot stay 24/7 in voice channel run `music.afk`", message.channel)
        message.guild.me.voice.channel.leave();// Si vous voulez que votre bot reste en voc 24h/24 et 7j/7, supprimez cette ligne
        message.client.queue.delete(message.guild.id);
      }
 ...
 let stream = null; 
	...
	let thing = new MessageEmbed()
				.setAuthor("Started Playing Music!", "https://cdn.discordapp.com/attachments/837747398926532609/1315432519310774362/Music.gif")
				.setThumbnail(song.img)
				.setColor("BLUE")
				.addField("Name", song.title, true)
				.addField("Duration", song.duration, true)
